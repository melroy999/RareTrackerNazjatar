		...
		function(_, button)
			...
			if button == "LeftButton" then
				...
				if self.current_health[npc_id] then
					...
					if loc then
						SendChatMessage(
							string.format(L["<RTN> %s (%s%%) seen at ~(%.2f, %.2f)"], name, health, loc.x, loc.y),
							target,
							nil,
							channel_id
						)
						...
						SendChatMessage(
							string.format(L["<RTN> %s (%s%%)"], name, health),
							target,
							nil,
							channel_id
						)
				...
				elseif self.last_recorded_death[npc_id] ~= nil then
					if GetServerTime() - last_death < 60 then
						SendChatMessage(
							string.format(L["<RTN> %s has died"], name, GetServerTime() - last_death),
							target,
							nil,
							channel_id
						)
						...
						SendChatMessage(
							string.format(
								L["<RTN> %s was last seen ~%s minutes ago"],
								name,
				...
				elseif self.is_alive[npc_id] then
					if loc then
						SendChatMessage(
							string.format(L["<RTN> %s seen alive, vignette at ~(%.2f, %.2f)"], name, loc.x, loc.y),
							target,
							nil,
							channel_id
						)
						...
						SendChatMessage(
							string.format(L["<RTN> %s seen alive (combat log)"], name),
							target,
							nil,
							channel_id
						)
	...
	f.reload_button:SetScript("OnClick",
		function()
			if self.current_shard_id ~= nil and GetServerTime() - self.last_reload_time > 600 then
				print(L["<RTN> Resetting current rare timers and requesting up-to-date data."])
				self.is_alive = {}
				self.current_health = {}
			...
			elseif self.current_shard_id == nil then
				print(L["<RTN> Please target a non-player entity prior to resetting, "..
						"such that the addon can determine the current shard id."])
			else
				print(L["<RTN> The reset button is on cooldown. Please note that a reset is not needed "..
...
function RTN:InitializeCloseButton(f)
	f.close_button = CreateFrame("Button", "RTN.close_button", f)
	...
	f.close_button.texture:SetTexture("Interface\\AddOns\\RareTrackerNazjatar\\Icons\\Cross.tga")
